...
namespace FF8.JSM.Format
{
    public static class FormatHelper
    {
        ...
        public sealed class Formatter
        {
            ...
            public Formatter YieldReturn()
            {
                Sw.Append("yield return ");
                Sw.EndWithEmptyLine = true;
            ...
            public PropertyFormatter Service(String serviceName)
            {
                String name = Sw.GetLocal(serviceName, $"{nameof(ServiceId)}.{serviceName}[@ctx]");
                Sw.Append(name);
                return new PropertyFormatter(this);
            }
        ...
        public sealed class StaticTypeFormatter
        {
            ...
            public PropertyFormatter Property(String propertyName, String index = null)
            {
                Formatter.Sw.Append(".");
        ...
        public sealed class PropertyFormatter
        {
            ...
            public PropertyFormatter Index(String index)
            {
                Formatter.Sw.Append("[");
                Formatter.Sw.Append(index);
                Formatter.Sw.Append("]");
        ...
        public sealed class MethodFormatter
        {
            ...
            public MethodFormatter Argument<T>(String argumentName, T argumentValue) where T : Enum
            {
                return ArgumentInternal(argumentName, $"{typeof(T).Name}.{argumentValue}");
            }

            public MethodFormatter Argument(String argumentName, Int32 argumentValue)
            {
                return ArgumentInternal(argumentName, argumentValue.ToString(CultureInfo.InvariantCulture));
            }

            public MethodFormatter Argument(String argumentName, Boolean argumentValue)
            {
                return ArgumentInternal(argumentName, argumentValue ? "true" : "false");
            }
            ...
            {
                return ArgumentInternal(argumentName, $"\"{argumentValue}\"");
            }

            private MethodFormatter ArgumentInternal(String argumentName, String argumentValue)
            {
                ScriptWriter sw = Formatter.Sw;
